...
function procesarMsgPersona(objMsg) {
  ...
  // GES_CDETALLE9: Codigo de servicio CUPS
  // GES_CDETALLE10: Tipo doc paciente
  // GES_CDETALLE11: Numero documento paciente
  // GES_CDETALLE12: Numero de autorizacion que remite EPS
  ...
  if (ARR_TEL_AUTORIZADOS.includes(TEL_PERSONA)) {
    ...
    sql = `SELECT PKGES_CODIGO, GES_CULT_MSGBOT, FKGES_NPER_CODIGO, GES_ESTADO_CASO FROM ${DB}.tbl_gestion WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
    connDB
      .promise()
      .query(sql, [TEL_PERSONA, 'Activo'])
      .then(([results]) => {
        if (results.length > 0) {
          ...
          switch (ULT_MSGBOT) {
            case 'MSG_SALUDO':
              if (TIPO_MSG_PERSONA === 'chat') {
                ...
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
            ...
            case 'MSG1':
              if (TIPO_MSG_PERSONA === 'chat') {
                ...
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE1 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
            ...
            case 'MSG2':
              if (TIPO_MSG_PERSONA === 'chat') {
                ...
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE2 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
            ...
            case 'MSG3':
              if (TIPO_MSG_PERSONA === 'chat') {
                ...
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE3 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
            ...
            case 'MSG4':
              ...
              if (MSG_PERSONA === 'SI' || MSG_PERSONA === 'NO') {
                ...
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE4 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
            ...
            case 'MSG5':
              if (MSG_PERSONA === 'SI' || MSG_PERSONA === 'NO') {
                ...
                sql = `SELECT GES_CSINTOMAS FROM ${DB}.tbl_gestion WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
                    ...
                    if (GUION_BOT.sintomas.length - 1 === TAMANO_JSON_SINTOMASDB) {
                      ...
                      sql = `UPDATE ${DB}.tbl_gestion SET GES_CSINTOMAS = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                      connDB
                        .promise()
                        .query(sql, [JSONParaGES_CSINTOMAS, TEL_PERSONA, 'Activo'])
                        .then(([results]) => {
                          if (results.affectedRows > 0) {
            ...
            case 'MSG6':
              if (TIPO_MSG_PERSONA === 'chat') {
                ...
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE6 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
            ...
            case 'MSG7':
              ...
              if (!isNaN(new Date(`${mes}-${dia}-${ano}`))) {
                // * true: es fecha
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE7 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
            ...
            case 'MSG8':
              ...
              if (!isNaN(new Date(`${mes}-${dia}-${ano}`))) {
                // * true: es fecha
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE8 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
                    if (results.affectedRows > 0) {
                      botHabla({ enviaMsg: 'MSG9', idPersona: ID_PERSONA, telPersona: TEL_PERSONA });
                    }
                  })
                  .catch((error) => console.log(error));
              } else {
            ...
            // todo, VALIDAR LAS DE AQUI PABAJO (hasta la 12), tipos de dato y eso
            case 'MSG9':
              // todo, validar que solo sea numerico
              if (TIPO_MSG_PERSONA === 'chat') {
                console.log(TIPO_MSG_PERSONA);
                console.log(MSG_PERSONA); // guarda rta de 'msg6' en DB -> (persona debe responder eps)
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE9 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
                    if (results.affectedRows > 0) {
                      botHabla({ enviaMsg: 'MSG10', idPersona: ID_PERSONA, telPersona: TEL_PERSONA });
                    }
                  })
                  .catch((error) => console.log(error));
              } else {
                botHabla({ enviaMsg: 'MSG9', idPersona: ID_PERSONA, telPersona: TEL_PERSONA, msgExtra: '🤔 Lo siento no entendí tu respuesta\n\n' });
              }
              break;

            case 'MSG10':
              if (TIPO_MSG_PERSONA === 'chat' || TIPO_MSG_PERSONA === 'list_response') {
                // guarda rta de 'msg6' en DB -> (persona debe responder eps)
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE10 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
                    if (results.affectedRows > 0) {
                      botHabla({ enviaMsg: 'MSG11', idPersona: ID_PERSONA, telPersona: TEL_PERSONA });
                    }
                  })
                  .catch((error) => console.log(error));
              } else {
                botHabla({ enviaMsg: 'MSG10', idPersona: ID_PERSONA, telPersona: TEL_PERSONA, msgExtra: '🤔 Lo siento no entendí tu respuesta\n\n' });
              }
              break;
            ...
            case 'MSG11':
              ...
              if (TIPO_MSG_PERSONA === 'chat') {
                // guarda rta de 'msg6' en DB -> (persona debe responder eps)
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE11 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
                    if (results.affectedRows > 0) {
                      botHabla({ enviaMsg: 'MSG12', idPersona: ID_PERSONA, telPersona: TEL_PERSONA });
                    }
                  })
                  .catch((error) => console.log(error));
              } else {
                botHabla({ enviaMsg: 'MSG11', idPersona: ID_PERSONA, telPersona: TEL_PERSONA, msgExtra: '🤔 Lo siento no entendí tu respuesta\n\n' });
              }
              break;
            ...
            case 'MSG12':
              ...
              if (TIPO_MSG_PERSONA === 'chat') {
                // guarda rta de 'msg6' en DB -> (persona debe responder eps)
                sql = `UPDATE ${DB}.tbl_gestion SET GES_CDETALLE12 = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
                connDB
                  .promise()
                  .query(sql, [MSG_PERSONA, TEL_PERSONA, 'Activo'])
                  .then(([results]) => {
                ...
                botHabla({ enviaMsg: 'MSG12', idPersona: ID_PERSONA, telPersona: TEL_PERSONA, msgExtra: '🤔 Lo siento no entendí tu respuesta\n\n' });
              }
              break;
            ...
            case 'MSG_FIN':
              ...
              // CERRADO: La persona ya se le cerró su interacción (su caso), si vuelve a hablar se updatea su registro como osi hablara por primera vez
              ...
              if (ESTADO_CASO === 'ABIERTO' && ID_AGENTE !== null) {
                ...
                sql = `INSERT INTO ${DB}.tbl_mensajes_chat (FK_GES_CODIGO, MEN_ESTADO_MENSAJE, MEN_NUMERO_ORIGEN, MEN_TEXTO, MEN_TIPO_MENSAJE, MEN_CESTADO) VALUES (?,?,?,?,?,?)`;
                connDB
                  .promise()
                  .query(sql, [PKGES_CODIGO, 'RECIBIDO', TEL_PERSONA, msgPersona, TIPO_MSG_PERSONA, 'Activo'])
                  .then(([results]) => { });
              }

              if (ESTADO_CASO === 'CERRADO' && ID_AGENTE !== null) {
                // Si la persona le vuelve a hablar y está en CERRADO:
                // Se hace un update del estado del registro a Inactivo
                // se le vuelva a enviar el primer mensaje SALUDO

                sqlUpdate = `UPDATE ${DB}.tbl_gestion SET GES_CESTADO = ? WHERE PKGES_CODIGO = ?`;
                connDB
                  .promise()
                  .query(sqlUpdate, ['Inactivo', PKGES_CODIGO])
                  .then(([results]) => {
                    if (results.affectedRows > 0) {
  ...
  switch (objData.enviaMsg) {
    case 'MSG_SALUDO':
      ...
      clientWP.sendMessage(objData.idPersona, GUION_BOT.saludoInicial).then(() => {
        sql = `INSERT INTO ${DB}.tbl_gestion (GES_NUMERO_COMUNICA, GES_CULT_MSGBOT, GES_CESTADO) VALUES (?, ?, ?)`;
        connDB
          .promise()
          .query(sql, [objData.telPersona, 'MSG_SALUDO', 'Activo'])
          .then(() => {
    ...
    case 'MSG_NOMBRE':
      ...
      clientWP.sendMessage(objData.idPersona, GUION_BOT.preguntaNombre).then(() => {
        sql = `UPDATE ${DB}.tbl_gestion SET GES_CULT_MSGBOT = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
        connDB
          .promise()
          .query(sql, ['MSG_SALUDO', objData.telPersona, 'Activo'])
          .then(() => {
    ...
    case 'MSG1':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg1}`).then(() => {
        ...
        connDB
          .promise()
          .query(sql, ['MSG1', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG2':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg2}`).then(() => {
        ...
        connDB
          .promise()
          .query(sql, ['MSG2', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG3':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg3}`).then(() => {
        ...
        connDB
          .promise()
          .query(sql, ['MSG3', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG4':
      ...
      clientWP.sendMessage(objData.idPersona, listaContactoCOVID).then(() => {
        ...
        connDB
          .promise()
          .query(sql, ['MSG4', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG5':
      ...
      connDB
        .promise()
        .query(sql, [objData.telPersona, 'Activo'])
        .then(([results]) => {
          const JSON_SINTOMASDB = results[0].GES_CSINTOMAS;
          ...
          if (JSON_SINTOMASDB === null) {
            ...
            clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg5}`).then(() => {
              ...
              clientWP.sendMessage(objData.idPersona, listaSintoma).then(() => {
                ...
                connDB
                  .promise()
                  .query(sql, ['MSG5', objData.telPersona, 'Activo'])
                  .then(([results]) => {
                    if (results.affectedRows > 0) {
                    }
                  })
    ...
    case 'MSG6':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg6}`).then(() => {
        ...
        connDB
          .promise()
          .query(sql, ['MSG6', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG7':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg7}`).then(() => {
        ...
        connDB
          .promise()
          .query(sql, ['MSG7', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG8':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg8}`).then(() => {
        ...
        connDB
          .promise()
          .query(sql, ['MSG8', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG9':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg9}`).then(() => {
        sql = `UPDATE ${DB}.tbl_gestion SET GES_CULT_MSGBOT = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
        connDB
          .promise()
          .query(sql, ['MSG9', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG10':
      const arrTiposDoc = [{ title: 'Cédula de ciudadania' }, { title: 'Tarjeta de identidad' }, { title: 'Registro civil' }, { title: 'Cédula de extranjería' }, { title: 'Tarjeta de extranjería' }, { title: 'NIT' }, { title: 'Pasaporte' }, { title: 'Tipo de documento extranjero' }, { title: 'Sin identificación del exterior o para uso definido por la DIAN. Cuantías menores' }];
      const sectionsTiposDoc = [{ title: 'Por favor seleccione una opción', rows: arrTiposDoc }];
      const listaTiposDoc = new List(`${MSG_EXTRA}${GUION_BOT.msg10}`, 'Por favor seleccione una opción', sectionsTiposDoc);

      clientWP.sendMessage(objData.idPersona, listaTiposDoc).then(() => {
        sql = `UPDATE ${DB}.tbl_gestion SET GES_CULT_MSGBOT = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
        connDB
          .promise()
          .query(sql, ['MSG10', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG11':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg11}`).then(() => {
        sql = `UPDATE ${DB}.tbl_gestion SET GES_CULT_MSGBOT = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
        connDB
          .promise()
          .query(sql, ['MSG11', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG12':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msg12}`).then(() => {
        sql = `UPDATE ${DB}.tbl_gestion SET GES_CULT_MSGBOT = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
        connDB
          .promise()
          .query(sql, ['MSG12', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG_FINCOVID':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msgFinCovid}`).then(() => {
        sql = `UPDATE ${DB}.tbl_gestion SET GES_CULT_MSGBOT = ?, GES_ESTADO_CASO = ?, FKGES_NPER_CODIGO = ? WHERE GES_NUMERO_COMUNICA = ? AND GES_CESTADO = ?`;
        connDB
          .promise()
          .query(sql, ['MSG_FINCOVID', 'CERRADO', 0, objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG_ESPERE':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${GUION_BOT.msgEspere}`).then(() => {
        ...
        connDB
          .promise()
          .query(sql, ['MSG_FIN', objData.telPersona, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
    ...
    case 'MSG_CHATWEB':
      clientWP.sendMessage(objData.idPersona, `${MSG_EXTRA}${MSG_CHATWEB}`).then(() => {
        sql = `UPDATE ${DB}.tbl_mensajes_chat SET MEN_ESTADO_MENSAJE = ? WHERE PKMEN_NCODIGO = ? AND MEN_CESTADO = ?`;
        connDB
          .promise()
          .query(sql, ['ENVIADO', ID_MSGWEB, 'Activo'])
          .then(([results]) => {
            if (results.affectedRows > 0) {
            }
          })
...
function recuBotEnviaMsgAgente() {
  ...
  sql = `SELECT PKMEN_NCODIGO, FK_GES_CODIGO, MEN_ESTADO_MENSAJE, MEN_NUMERO_ORIGEN, MEN_NUMERO_DESTINO, MEN_TEXTO FROM ${DB}.tbl_mensajes_chat WHERE MEN_ESTADO_MENSAJE = ? AND MEN_CESTADO = ?`;
  connDB
    .promise()
    ...
    .then(([results]) => {
      if (results.length > 0) {
        ...
        if (MSG_CHATWEB === TEXTO_REPETIDO) {
          sql = `SELECT COUNT(PKMEN_NCODIGO) AS CANT_REPETIDOS FROM ${DB}.tbl_mensajes_chat WHERE MEN_TEXTO = ? AND FK_GES_CODIGO = ? AND MEN_CESTADO = ?`;
          connDB
            .promise()
            .query(sql, [TEXTO_REPETIDO, FK_GES_CODIGO, 'Activo'])
            .then(([results]) => {
